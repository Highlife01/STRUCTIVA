        ...
        <div className="relative z-10 mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 py-24 sm:py-32 w-full">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2.5 rounded-full bg-amber-500/20 border border-amber-500/40 px-4 py-1.5 text-[11px] font-bold uppercase tracking-widest text-amber-300 mb-6 backdrop-blur-sm">
              ...
              <span>STRUCTIVA GLOBAL MÜHENDİSLİK · ADANA TESİSLERİ</span>
            </div>
          ...
          <div className="mt-16 sm:mt-24 grid grid-cols-2 sm:grid-cols-4 gap-4 border-t border-white/15 pt-8 max-w-4xl bg-black/40 backdrop-blur-md rounded-2xl p-6 border">
            <div>
              ...
              <span className="text-xs text-slate-300 uppercase tracking-wider">{t("stat_clear_span")}</span>
            </div>
            <div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div>
              <span className="text-amber-400 text-xs font-mono font-bold uppercase tracking-widest block mb-2">
                Canlı Proje Galerisi
              </span>
              <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight">
                HD Çelik Yapı ve <span className="text-amber-400">Hangar Görüntüleri</span>
              </h2>
            </div>
            <p className="text-xs text-slate-400 max-w-md">
              Büyük ölçekli kemerli çelik hangarlarımızın, cıvatalı montaj detaylarımızın ve küresel antrepo sahalarımızın yüksek çözünürlüklü fotoğrafları.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-5 space-y-6">
              <span className="text-amber-400 text-xs font-mono font-bold uppercase tracking-widest block">
                Fabrika & İmalat Filmi
              </span>
              <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight">
                Structiva Tesisleri Adana <br />
                <span className="text-amber-400">Üretim & Montaj Gücü</span>
              </h2>
              <p className="text-sm text-slate-300 leading-relaxed">
                Adana'daki entegre üretim merkezimizde yüksek hassasiyetli roll-forming, 
                CNC plazma kesim ve çift taraflı Galvalume korumalı kemer panellerinin 
                üretim ve cıvatalı montaj aşamalarını izleyin.
              </p>
              <div className="space-y-3 pt-2">
                <div className="flex items-center gap-3 text-xs text-slate-300">
                  <CheckCircle2 size={16} className="text-emerald-400 shrink-0" />
                  <span>Sıfır şantiye kaynağı ile hızlı cıvatalı kurulum</span>
                </div>
                <div className="flex items-center gap-3 text-xs text-slate-300">
                  <CheckCircle2 size={16} className="text-emerald-400 shrink-0" />
                  <span>240 km/h rüzgar ve sismik dayanım testleri</span>
                </div>
                <div className="flex items-center gap-3 text-xs text-slate-300">
                  <CheckCircle2 size={16} className="text-emerald-400 shrink-0" />
                  <span>Flat-pack deniz aşırı konteyner sevkiyatı</span>
                </div>
              </div>
              ...
              <div className="pt-4">
                ...
                >
                  <span>Kendi Binanızı Tasarlayın</span>
                  <MoveRight size={14} />
                </Link>
              </div>
            </div>
              ...
              <div className="rounded-3xl overflow-hidden border border-amber-500/30 shadow-2xl relative bg-black">
                ...
                <div className="p-4 bg-[#0d1822] border-t border-white/10 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs font-semibold text-white">
                    ...
                    <span>Structiva Tesisleri Adana Tanıtım Videosu</span>
                  </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
            <div>
              ...
              <h2 className="text-3xl sm:text-5xl font-black tracking-tight text-white">
                Mühendislik Harikası <span className="text-slate-400">Çelik Modeller</span>
              </h2>
            </div>
            <Link href="/models" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-amber-400 hover:text-white transition-colors">
              <span>Tüm Modelleri & Çizimleri İncele</span>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 border-b border-white/10">
        <div className="mx-auto max-w-[1440px]">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
            <div>
              <span className="text-amber-400 text-xs font-mono font-bold uppercase tracking-widest block mb-2">
                02 / Endüstriyel Kullanım
              </span>
              <h2 className="text-3xl sm:text-5xl font-black tracking-tight text-white">
                Her Sektörün <span className="text-slate-400">Kendi Mantığı Vardır</span>
              </h2>
            </div>
            <Link href="/sectors" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-amber-400 hover:text-white transition-colors">
              <span>Tüm Sektör Çözümleri</span>
              <ArrowUpRight size={16} />
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {SECTORS.map((sec) => (
              <div key={sec.id} className="rounded-2xl bg-[#0e1b27] border border-white/10 p-6 hover:border-white/20 transition-all flex flex-col justify-between">
                ...
                >
                  <span>Bu Sektör İçin Teklif İste</span>
                  <MoveRight size={13} />
                </Link>
              </div>
            ))}
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 border-b border-white/10 bg-[#0b1622]">
        <div className="mx-auto max-w-[1440px] grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-6">
            <span className="text-amber-400 text-xs font-mono font-bold uppercase tracking-widest block mb-2">
              03 / Üretim & İhracat Gücü
            </span>
            <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight mb-6">
              Structiva Tesisleri Adana'dan <br />
              <span className="text-amber-400">Dünyanın Dört Bir Yanına</span>
            </h2>
            <p className="text-sm text-slate-300 leading-relaxed mb-6">
              STRUCTIVA, Adana merkezli entegre çelik üretim tesisleri ve Akdeniz liman lojistiği avantajıyla 
              50'den fazla ülkeye demonte (flat-pack) kemerli ve endüstriyel çelik bina kitleri ihraç etmektedir.
            </p>
            <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6 text-xs text-slate-300">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <strong className="block text-white font-bold">Doğrudan Konteyner Sevkiyatı</strong>
                  <span>Tüm modeller tek 40HC konteynerde 350-400 m² paketlenir.</span>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <ShieldCheck size={20} className="text-emerald-400 shrink-0" />
                <div>
                  <strong className="block text-white font-bold">Avrupa & ABD Standartları</strong>
                  <span>EN 1090-2 EXC4 CE ve AISC normlarına tam uygunluk.</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 relative">
            ...
            <div className="absolute -bottom-6 -left-6 bg-[#0f1d2a] border border-amber-500/40 p-5 rounded-2xl shadow-2xl hidden sm:block">
              <div className="text-amber-400 font-mono text-xs uppercase font-bold mb-1">Adana Entegre Üretim</div>
              <div className="text-white text-lg font-black">Structiva Tesisleri Adana</div>
              <div className="text-slate-400 text-xs">Yıllık 25.000 ton çelik işleme kapasitesi</div>
            </div>
          </div>
        </div>
      </section>
